...
def test_brand_profile_crud():
    ...
    pid = _make_project()
    session = get_session()
    bp = BrandProfile(project_id=pid, brand_tone="极简高端", color_system="黑白灰")
    session.add(bp)
    session.commit()

    loaded = session.query(BrandProfile).filter_by(project_id=pid).first()
    assert loaded is not None
    assert loaded.brand_tone == "极简高端"
    assert loaded.color_system == "黑白灰"
    assert loaded.photo_style is None  # nullable
    session.close()
...
def test_build_brand_profile_creates_empty_if_missing():
    """不存在时 build_brand_profile 自动创建空记录"""
    from pipeline.layers.brand_profiler import build_brand_profile

    pid = _make_project()
    bp = build_brand_profile(pid)
    assert bp is not None
    assert bp.project_id == pid
    assert bp.brand_tone is None


def test_build_brand_profile_returns_existing():
    """已存在时返回现有记录"""
    from pipeline.models.brand_profile import BrandProfile
    from pipeline.models.base import get_session
    from pipeline.layers.brand_profiler import build_brand_profile
    ...
    pid = _make_project()
    session = get_session()
    session.add(BrandProfile(project_id=pid, brand_tone="复古"))
    session.commit()
    session.close()
