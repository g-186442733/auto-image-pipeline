...
class TestConfidenceRouter:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (100, "pass"),
            (80.0, "pass"),
            (79.9, "retry_alt_prompt"),
            (65, "retry_alt_prompt"),
            (50.0, "retry_alt_prompt"),
            (49.9, "human_review"),
            (30, "human_review"),
            (0, "human_review"),
        ],
    )
    def test_route(self, router, score, expected):
        assert router.route(score) == expected
    ...
    def test_is_pass(self, router, score, expected):
        assert router.is_pass(score) == expected
    ...
    def test_is_human_review(self, router, score, expected):
        assert router.is_human_review(score) == expected
