...
import pipeline.models.flywheel_observation
...
def test_fresh_migration_adds_all_columns(fresh_engine):
    ...
    assert "flywheel_observations" in inspect(fresh_engine).get_table_names()
    observation_cols = _col_names(fresh_engine, "flywheel_observations")
    assert "source_type" in observation_cols
    assert "combined_score" in observation_cols
    assert "visual_tags" in observation_cols
