...
def analyze_price(
    asin: str, keepa_data: dict | None, competitor_prices: list[float] | None
) -> PriceAnalysis:
    keepa_data = keepa_data or {}
    benchmark_prices = [p for p in (competitor_prices or []) if p and p > 0]
    ...
    return PriceAnalysis(
        ...
        competitor_prices=str(benchmark_prices) if benchmark_prices else None,
    )
