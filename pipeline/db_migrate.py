...
MIGRATIONS = [
    ...
    ("prompt_assets", "prompt_text_zh", "TEXT DEFAULT NULL"),
    ...
    # A+ 专属生图：宽幅图片路径、实际发送的 prompt、生成尺寸
    ("aplus_contents", "image_path", "VARCHAR(500) DEFAULT NULL"),
    ("aplus_contents", "image_prompt", "TEXT DEFAULT NULL"),
    ("aplus_contents", "image_size", "VARCHAR(20) DEFAULT NULL"),
    # A/B 实测原始数据：保留 CTR/CVR 供 WebUI 回显
    ("prompt_assets", "ab_ctr", "FLOAT DEFAULT NULL"),
    ("prompt_assets", "ab_cvr", "FLOAT DEFAULT NULL"),
    # Task 5: A+ Content QA 与人工审批字段
    ("aplus_contents", "qa_score", "FLOAT DEFAULT NULL"),
    ("aplus_contents", "qa_passed", "BOOLEAN DEFAULT FALSE"),
    ("aplus_contents", "qa_issues", "TEXT DEFAULT NULL"),
    ("aplus_contents", "retry_count", "INTEGER DEFAULT 0"),
    ("aplus_contents", "approved", "BOOLEAN DEFAULT FALSE"),
    ("aplus_contents", "rejected", "BOOLEAN DEFAULT FALSE"),
    ("aplus_contents", "approved_at", "TIMESTAMP DEFAULT NULL"),
    # P1: 套图 Reject 支持
    ("prompt_assets", "rejected", "BOOLEAN DEFAULT FALSE"),
    # P2: A+ 人工评分多维度扩展
    ("human_aplus_scores", "score_image_quality", "FLOAT DEFAULT NULL"),
    ("human_aplus_scores", "score_copy_quality", "FLOAT DEFAULT NULL"),
    ("human_aplus_scores", "score_layout", "FLOAT DEFAULT NULL"),
    ("human_aplus_scores", "score_brand_fit", "FLOAT DEFAULT NULL"),
    ("human_aplus_scores", "score_conversion", "FLOAT DEFAULT NULL"),
    ("human_aplus_scores", "overall_score", "FLOAT DEFAULT NULL"),
    ("human_aplus_scores", "failure_tags", "TEXT DEFAULT NULL"),
    # 生成后回写视觉特征（vision_analyzer 分析结果，供 F4 避重和色彩锚定使用）
    ("slot_plans", "generated_lighting", "VARCHAR(50) DEFAULT NULL"),
    ("slot_plans", "generated_angle", "VARCHAR(30) DEFAULT NULL"),
    ("slot_plans", "generated_shot_type", "VARCHAR(50) DEFAULT NULL"),
    ("slot_plans", "generated_bg_material", "VARCHAR(50) DEFAULT NULL"),
    ("slot_plans", "generated_color_temp", "VARCHAR(30) DEFAULT NULL"),
    ("slot_plans", "generated_saturation", "VARCHAR(20) DEFAULT NULL"),
]
...
POSTGRES_CREATE_TABLES = [
    ...
    """CREATE TABLE IF NOT EXISTS human_aplus_scores (
        id SERIAL PRIMARY KEY,
        project_id INTEGER NOT NULL REFERENCES projects(id),
        module_id INTEGER NOT NULL REFERENCES aplus_contents(id),
        score INTEGER NOT NULL CHECK (score >= 1 AND score <= 5),
        comment TEXT,
        created_at TIMESTAMP DEFAULT NOW()
    )""",
    """CREATE TABLE IF NOT EXISTS flywheel_observations (
        id SERIAL PRIMARY KEY,
        project_id INTEGER NOT NULL REFERENCES projects(id),
        prompt_asset_id INTEGER REFERENCES prompt_assets(id),
        aplus_content_id INTEGER REFERENCES aplus_contents(id),
        slot_index INTEGER,
...
def run_migrations(engine):
    with engine.connect() as conn:
        ...
        for col in [
            ...
            if col not in _get_columns(conn, "customer_briefs"):
                conn.execute(text(f"ALTER TABLE customer_briefs ADD COLUMN {col} TEXT"))
