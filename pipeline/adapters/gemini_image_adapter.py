...
_OFFICIAL_FALLBACK_MODEL = "gpt-image-2"


def _parse_size(params: dict | None) -> tuple[int, int] | None:
    if not params:
        return None
...
class GeminiImageAdapter(BaseImageAdapter):
    ...
    def _save_image(
        ...
        if target_wh:
            # 强制 resize 到目标宽高（API 无法控制输出尺寸，生成后矫正）
            if img.size != target_wh:
                img = img.resize(target_wh, PilImage.Resampling.LANCZOS)
        else:
            # 兜底：按 config 最大边缩小，保持比例
    ...
    def edit_image(
        ...
        if primary_model.endswith("-client"):
        ...
        try:
            ...
            try:
                return GeminiImageAdapter().edit_image(
                    image_b64=image_b64, prompt=prompt, params=fb_params
                )
            except Exception as gemini_exc:
                logger.error(
                    "Gemini fallback failed (%s), falling back to official %s",
                    gemini_exc,
                    _OFFICIAL_FALLBACK_MODEL,
                )
                return GeminiImageAdapter().edit_image(
                    image_b64=image_b64,
                    prompt=prompt,
                    params={"model": _OFFICIAL_FALLBACK_MODEL},
                )

        job_id = uuid.uuid4().hex[:12]
        out_path: str | None = None
        target_wh = _parse_size(params)
        ...
        if match:
            ...
            out_path = self._save_image(img_data, fmt, out_dir, job_id, target_wh)
            logger.info(
                "Gemini image saved: %s (%dx%d)",
                out_path,
                *PilImage.open(out_path).size,
            )
        else:
            logger.error(
    ...
    def edit(
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    "Gemini edit fallback failed (%s), falling back to official %s",
                    gemini_exc,
                    _OFFICIAL_FALLBACK_MODEL,
                )
                return GeminiImageAdapter().edit(
                    image_paths, prompt, params={"model": _OFFICIAL_FALLBACK_MODEL}
                )
