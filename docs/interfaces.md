...
def create_project(brief: dict) -> Project:
    ...
    Args:
        brief: 项目信息字典，必需字段:
            - name (str): 项目名称
            - category (str): 品类（Keepa catId 字符串，来源 L1+L2 级联选择器，数据源 amazon_categories.py）
            可选字段:
            - asin (str): 目标 ASIN（非空时校验格式：B0 开头、长度 10）
            - notes (str): 备注
    ...
    Raises:
        E_INPUT_001: brief 缺少必需字段 (name/category)
        E_INPUT_002: ASIN 格式校验失败 (非 B0 开头或长度不对，仅当 asin 非空时触发)
    """
...
def record_ab_test(
    ...
    注意：此函数为外部/手动触发接口，不由 step_feedback() 自动调用。
    需由运营人员在测试完成后手动调用或通过 Web UI 提交。
...
GET  /input/new           # 新建项目表单；模板变量: amazon_category_tree (JSON, L1+L2 树形结构)
POST /input/new           # 提交新项目；product_category 字段接受 Keepa catId 字符串 → input_layer.create_project
GET  /prompts             # Prompt 资产浏览/管理
GET  /benchmarks          # 竞品基准数据浏览
POST /project/<id>/run    # 触发全流程执行
GET  /project/<id>/report # 项目报告 (JSON)
GET  /customers           # 客户管理页面：列出所有客户
POST /customers/new       # 新建客户
GET  /api/customers/search?q=  # 客户搜索 JSON API，返回匹配客户列表
GET  /knowledge           # 知识库页面；支持 ?q=搜索词&category=分类 筛选
POST /knowledge/new       # 新建知识条目；表单字段: category(prompt_pattern|qa_lesson|style_rule|client_preference), title, content, tags, source_project_id(可选)
POST /knowledge/<id>/delete  # 删除知识条目（按 entry_id + tenant_id 双重校验）
```
...
| 错误码           | 模块            | 描述                                           |
| ---------------- | --------------- | ---------------------------------------------- |
| `E_INPUT_001`    | input_layer     | 项目 brief 缺少必需字段 (name/category)        |
...
def apply_attribution(session, data: list[dict]) -> int:
    """将归因结果写入 PromptAsset.performance_score，并记录飞轮观察。

    每条有效数据会在更新 PromptAsset 后写入一条
    FlywheelObservation(source_type="ab_performance")，performance_details 中保留
    ctr / cvr / performance_score，供后续归档和复盘使用。
...
def run_flywheel(
    ...
    """执行飞轮归档：将高质量 PromptAsset 归档为可复用样本。

    当前飞轮不再表示“低 QA 分自动触发 DeliveryVersion 重生成”。统一反馈来源
    先写入 FlywheelObservation；run_flywheel 负责把已通过 QA 且综合分达标的
    Listing Prompt 归档到 FlywheelExample，并创建 source="flywheel" 的
    PromptAsset 兼容副本。
    ...
    Args:
        ...
        tenant_id: 兼容旧多租户字段，可选

    Returns:
        dict:
            archived (int): 本次归档成功的样本数
            skipped (int): 因无通过 QA 或未达阈值跳过的资产数
            skipped/reason: 飞轮关闭时返回 {"skipped": True, "reason": "disabled"}

    Notes:
        - Listing slot_type 标准化为 MAIN / ALT1~ALT7，确保 prompt_engine 可读取。
        - 冷启动阈值为 3.5；同品类样本数 ≥30 后阈值为 4.0。
        - source="flywheel" 副本会复制 visual_tags，供 BrandProfile ELASTIC 写回。
    """
...
def check_flywheel_status(config: Config | None = None) -> dict:
    ...
    Returns:
        dict:
            enabled (bool): 飞轮是否启用
            auto_deliver (bool): 兼容配置项
            confidence_threshold (float): 兼容置信度阈值配置
    """
```

### flywheel_observation — 统一观察层

```python
# pipeline/layers/flywheel_observation.py
def slot_label(slot_index: int | None) -> str | None:
    """将 slot_index 标准化为 MAIN / ALT1~ALT7。"""

def record_listing_qa_observation(session, asset, qa_record, slot_plan=None) -> FlywheelObservation:
    """将 Listing QARecord 写入 FlywheelObservation(source_type='listing_qa')。

    QA 0~100 分会除以 20 归一化为 0~5；qa_details / delivery_status /
    reference_basis / visual_tags 会一起保留。
    """

def record_listing_human_observation(session, asset, human_score, slot_plan=None) -> FlywheelObservation:
    """将 WebUI 人审 HumanImageScore 写入 FlywheelObservation(source_type='listing_human')。"""

def record_aplus_observation(session, module, human_scores=None) -> FlywheelObservation | None:
    """将 A+ QA/人审结果写入 FlywheelObservation(source_type='aplus_qa')。

    仅当 APlusContent 有 image_prompt 且有 qa_score 时写入；slot_type 使用
    APLUS_<module_type>。
    """

def record_ab_performance_observation(
    session,
    asset,
    *,
    ctr: float,
    ...
    """将 A/B CTR/CVR 归因写入 FlywheelObservation(source_type='ab_performance')。"""

def archive_observation_to_flywheel(
    session,
    observation: FlywheelObservation,
    *,
    threshold: float = 4.0,
) -> FlywheelExample | None:
    """当 observation.combined_score 达标时，创建兼容旧链路的 FlywheelExample。"""

def copy_visual_tags_to_flywheel_asset(source_asset: PromptAsset, flywheel_asset: PromptAsset) -> None:
    """复制 visual_tags，保证 flywheel PromptAsset 可用于品牌 ELASTIC 写回。"""
```
...
@dataclass
class Config:
    db_path: str            # AIP_DB_PATH, legacy/test path, default "data/pipeline.db"
    db_url: str             # AIP_DB_URL, default "postgresql://localhost/aip_db"
...
## Wave 3 — 产品图 Vision Conditioning（v1.6）

### CustomerBrief（更新）

新增字段（对应 `pipeline/models/customer_brief.py` 及数据库迁移 Wave 3）：

```python
class CustomerBrief(Base):
    # ... 原有字段 ...
    product_dimensions: str | None       # 产品尺寸
    product_weight: str | None           # 产品重量
    product_material: str | None         # 材质
    product_color: str | None            # 颜色
    package_contents: str | None         # 包装内容
    product_certifications: str | None   # 认证信息
    listing_title: str | None            # 主标题
    listing_keywords: str | None         # 关键词（逗号分隔）
    listing_bullets: str | None          # Bullet Points（换行分隔）
    white_bg_image_path: str | None      # 白底图本地路径（必填，上传后写入）
    multiangle_image_paths: str | None   # 多角度图路径列表（JSON 序列化，可选）
```

### GptImageAdapter.edit()（更新）

支持多张参考图作为 conditioning 输入：

```python
def edit(
    self,
    prompt: str,
    images: list[str],        # 本地图片路径列表，第一张为主参考图（白底图）
    mask: str | None = None,
...
```python
def step_generate(brief: CustomerBrief, slot: SlotPlan) -> ImageResult:
    """
    从 brief.white_bg_image_path 和 brief.multiangle_image_paths
    构建图片列表，调用 GptImageAdapter.edit() 进行多图 conditioning 生成。
    """
```
