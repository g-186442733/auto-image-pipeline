...
| 维度     | 内容                                                                                                             |
| -------- | ---------------------------------------------------------------------------------------------------------------- |
| **输入** | 品牌名称、主色调（十六进制色值）、字体风格、产品类目、目标市场（如 Amazon US）、竞品 ASIN 列表、参考图文件       |
| **处理** | 表单验证与参数归一化；参考图写入 `data/images/refs/`；project 记录和 brand_profile 记录写库                      |
...
| 维度     | 内容                                                                                                                                                                                                |
| -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **输入** | `brand_profiles` 记录（引擎C）、`amazon_benchmarks` Vision 报告（引擎A）、SLOT_MAPPING 配置（MAIN + PT01~PT07）                                                                                     |
| **处理** | Slot Plan 生成器按类目卖点矩阵规划 8 个 Slot；Prompt 组装引擎结合品牌画像、Vision 报告、INTENT_TAGS（6个）和 ROLE_TAGS（7个）拼装最终 Prompt；AI 适配器调用出图后端；标签关联写入 `tag_assignments` |
...
| 维度     | 内容                                                                                                                                          |
| -------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| **输入** | A/B 测试结果（CTR、CVR、自然排名变化）、对应的 slot_plan_id 和 prompt_asset_id                                                                |
| **处理** | 引擎B：写入 ABTest 记录；加权计算 Prompt 资产效果分；效果分达阈值时标记 `is_recommended = True`；低效方案降权；通知分析决策层刷新竞品基准权重 |
| **输出** | `ab_tests` 记录；`prompt_assets.performance_score` 更新；效果归因日志（高效模板沉淀为下一 SKU 的推荐起点）                                    |
...
projects (1) ─────────────── (1) brand_profiles
    ...
    │             │
    │             └── (1:1) qa_records   [每个 Slot 对应一条质检记录]
    │
...
| slot_type         | TEXT       | 适用 Slot 类型（MAIN / PT01~PT07）        |
| performance_score | FLOAT      | 效果分，由引擎B 回流数据计算，初始为 NULL |
| is_recommended    | BOOLEAN    | 是否标记为推荐模板                        |
| created_at        | DATETIME   | 创建时间                                  |
| updated_at        | DATETIME   | 最后更新时间                              |

**作用**：Prompt 资产库的核心表，支持版本管理和效果归因。引擎B 的 A/B 结果最终体现为 `performance_score` 字段更新，高分资产自动晋升为推荐模板。
...
A/B 测试数据回收后，`ab_tests` 写入新记录，`prompt_assets.performance_score` 滚动更新。效果分计算示例：

```
performance_score = 0.6 × CTR_normalized + 0.4 × CVR_normalized
```

权重可通过配置调整。效果分高于阈值（默认 0.75）的资产自动标记 `is_recommended = True`，成为下一个同类 SKU 的 Prompt 组装起点。
...
- SQLite 数据库文件保留完整历史，不做物理删除，用于分析和复盘
- L3 阶段迁移至 PostgreSQL 后，归档数据按租户隔离策略迁移
