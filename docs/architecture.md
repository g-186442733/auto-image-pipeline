...
| SQLAlchemy ORM       | T3       | 定义所有数据表（Project、BrandProfile、AmazonBenchmark、PromptAsset、SlotPlan、QARecord、ABTest、FlywheelObservation、TagAssignment），管理数据库会话 | 无                                | `pipeline/models/*.py`，默认 PostgreSQL，SQLite 仅用于测试兼容    | T1, T2         |
...
| ab_attribution       | L4-T4    | A/B 测试归因引擎：导入 CTR/CVR 数据，加权计算 performance_score（0.6×CTR + 0.4×CVR），写入 PromptAsset 与 FlywheelObservation      | ABTest 记录 / CSV 文件            | PromptAsset.performance_score 更新、ab_performance observation    | PromptAsset    |
...
| flywheel             | L4-T6    | 兼容归档飞轮：统一读取 QA/人审/A+、A/B observation，高分样本归档为 FlywheelExample + PromptAsset(source=flywheel)                 | FlywheelObservation / project_id  | 高分样本归档、visual_tags 保留、供下一轮 prompt 注入              | qa_gate / A+ / ab_attribution |
...
| 数据库       | PostgreSQL                                    | 当前真实运行默认 `postgresql://localhost/aip_db`，支持更可靠的并发与长期回流；SQLite 仅保留给既有测试兼容 | SQLite（仅测试/临时本地文件），MySQL                   |
...
│   │   └── amazon_categories.py      # L1+L2 树形结构，21个L1品类、约170个L2子品类，硬编码 Amazon US 分类，供 Web 级联选择器消费 (T2)
│   │
...
│   │   └── flywheel_observation.py   # FlywheelObservation 表：QA/人审/A+/A-B 统一观察层 (L4)
│   │
...
├── tests/                            # 测试套件
│   ├── unit/                         # 单元测试，无 IO，快速
│   ├── integration/                  # 集成测试，主要使用 SQLite 内存库兼容验证
│   └── e2e/                          # 端到端验证脚本 (T16)
│
├── data/                             # 运行时数据目录（不入 Git）
│   ├── pipeline.db                   # legacy/test SQLite 数据库文件（真实运行默认 PostgreSQL）
│   ├── images/                       # 生成图片存储
│   └── exports/                      # 交付包导出
│
...
- 真实运行默认 PostgreSQL；SQLite 仅保留测试兼容，不作为当前默认运行 DB
...
- PostgreSQL 已作为默认运行 DB；该阶段重点转为任务队列、连接池和多用户并发治理
...
**触发条件**：QA、人审、A+、A/B 等反馈来源可以稳定写入数据库。

当前 L4 的核心不是把 QA/A+/WebUI/A-B 打散重写，而是先统一观察层：

```text
QARecord / HumanImageScore / APlusContent / A-B attribution
        ↓
FlywheelObservation（统一 source_type、slot_type、score、visual_tags、delivery_status）
        ↓
FlywheelExample 兼容层 + PromptAsset(source="flywheel") + BrandProfile ELASTIC + category_priors
        ↓
prompt_engine.generate_slot_prompts 下一轮注入
```

在 L3 基础上新增：

- 数据回流层统一写入 `flywheel_observations`，避免 QA、A+、人审和 A/B 各自孤立沉淀
- 高分 observation 归档为 `flywheel_examples`，保持旧 prompt 注入链路兼容
- flywheel PromptAsset 副本保留 `visual_tags`，供品牌 ELASTIC 字段聚合写回
- 效果差的 Slot 方案先记录 observation / failure_tags，不再用“低 QA 分自动 DeliveryVersion”作为唯一飞轮定义
...
- Flask Web UI 页面响应目标 < 1 秒（PostgreSQL 本地连接在当前规模下足够；测试仍可用 SQLite 内存库）

### 存储

- PostgreSQL `aip_db` 保存真实运行数据；SQLite 文件只作为 legacy/test 兼容路径
...
- PostgreSQL 连接串通过 `AIP_DB_URL` 注入，默认 `postgresql://localhost/aip_db`；本地 SQLite 测试文件列入 `.gitignore`，不提交至版本库
- Flask 仅监听 `127.0.0.1`，L2 阶段不对外暴露端口
- 系统无用户认证层，为单管理员本地工具，不计划引入 Flask-Login
